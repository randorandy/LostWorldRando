...
def write_rom(game: Game, romWriter: Optional[RomWriter] = None) -> str:
    ...
    rom_name = f"LostWorld{game.seed}.sfc"
    rom1_path = f"roms/{rom_name}"
    rom_clean_path = "roms/Lost World.sfc"
    ...
    #Fix Swamp H3 Shaft Missile ID
    romWriter.writeBytes(0x7c487, b"\x8c")
    #Outside Tourian Room StateFix
    romWriter.writeBytes(0x7df5b, b"\x00\x00\x94\x9d\x5c\xe9\x70\x92")
    romWriter.writeBytes(0x7df6b, b"\x54\x87")
    #Fix Screw Missile
    romWriter.writeBytes(0x7c362, b"\x19")
    #Fix Swamp H4 Super ID
    romWriter.writeBytes(0x7c441, b"\x98")
    #Fix East of Springball respawning
    romWriter.writeBytes(0x784f1, b"\x00")
